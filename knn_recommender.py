...
class KnnRecommender:
    ...
    def make_recommendations(self, fav_movie, n_recommendations):
        ...
        # get data
        try:
            movie_user_mat_sparse = pickle.load(open("movie_user_mat_sparse.p", "rb"))
            hashmap = pickle.load(open("hashmap.p", "rb"))
        except:
            movie_user_mat_sparse, hashmap = self._prep_data()
            pickle.dump(movie_user_mat_sparse, open( "movie_user_mat_sparse.p", "wb"))
            pickle.dump(hashmap, open("hashmap.p", "wb"))
        # get recommendations
        ...
        output = f'Recommendations for {fav_movie}:\n\n'
        for i, (idx, dist) in enumerate(raw_recommends):
            output += f'{i+1}: {reverse_hashmap[idx]}, with distance of {dist}\n\n'
        return output
